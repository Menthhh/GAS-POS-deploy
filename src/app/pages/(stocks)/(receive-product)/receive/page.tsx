...
import useFetchReceives from "@/hooks/stock-product/receive/useFetchReceives";
...
function ReceivePage() {
  ...
  const { receives } = useFetchReceives(refresh);
  ...
  const filterAndSortReceiveByDate = (data) => {
    return data.filter((item) => {
        if (!dateFilter.fromDate && !dateFilter.toDate) return true;

        const itemDate = thaiDateToJSDate(item.RDATE);
      ...
      .sort((a, b) => {
        const dateA = thaiDateToJSDate(a.RDATE);
        const dateB = thaiDateToJSDate(b.RDATE);
        return dateB.getTime() - dateA.getTime(); // Descending order
      });
  };

  const filteredReceive = filterAndSortReceiveByDate(receives);
  const data = {
    headers: [
      <input
        type="checkbox"
        onChange={(e) => {
          ...
          if (e.target.checked) {
            setSelectedItems(receives.map((receieve) => receieve.REFNO));
          } else {
            setSelectedItems([]);
          }
        }}
        checked={selectedItems.length === receives.length}
      />,
      "ลำดับ",
      "เลขที่",
      "วันที่",
    ...
    rows: filteredReceive.map((item, index) => [
      ...
      <Link href={{
        pathname: "/pages/receive-edit-form",
