...
import { formData, SelectedProduct } from "./type/form.type";
...
function RecieveFormPage({searchParams} : any) {
  const refno = searchParams.refno

  
  const [refresh, setRefresh] = useState(false);
  const { receive, error } = useFetchReceive(refno, refresh);
  ...
  useEffect(() => {
    // Convert Buddhist date '07/07/2567' to Gregorian date
    const buddhistDate = receive.date? receive.date : formatDate(new Date());
    const [day, month, buddhistYear] = buddhistDate.split('/').map(Number);
    const gregorianYear = buddhistYear - 543;
    const formattedDate = formatDate(new Date(gregorianYear, month - 1, day));

    setFormData({
      refno: receive.refno || "",
      date: formattedDate || "",
      supplierId: receive.supplierId || "",
      selectedProducts: receive.selectedProducts || [],
      taxType: receive.taxType || "",
      totalBeforeTax: receive.totalBeforeTax || 0,
      taxPrice: receive.taxPrice || 0,
      diff: receive.diff || 0,
      totalWithTax: receive.totalWithTax || 0,
      totalPrice: receive.totalPrice || 0,
    });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("/api/update-receive", {
        method: "PUT",
  ...
  return (
    <Layout currentPage="สต็อกสินค้า" subPage="รับสินค้า">
      <form className="px-5 py-2 mt-2" onSubmit={handleSubmit}>
        <div className="flex items-center justify-center py-2 bg-white rounded-2xl mb-2">
          <div
            id="recieve-form"
        ...
        <div className="flex flex-col items-center justify-center p-5 bg-white rounded-2xl">
          <div
                    ...
                    <tr key={product.pid}>
                      ...
                      <td className="border border-end px-4 py-1">
                        <div className="flex justify-center items-center">
                          <input
                            type="number"
                            value={product.unitPrice.toFixed(1)}
                            onChange={(e) =>
                              handleProductChange(
                                index,
                                "unitPrice",
...
export default RecieveFormPage;