...
import useFetchCheckStock from "@/hooks/stock-product/check-stock/useFetchCheckStock";
import { CheckStock } from "./check-stock.type";


const columnWidths = ["5%", "5%", "10%", "10%", "15%"];
const startColumns: number[] = [];
const searchColumns: number[] = [2];
...
function ProductEditPage() {
  ...
  const handleDeleteAll = async () => {
    try {
      const response = await fetch("/api/delete-all-Improve-product", {
        method: "DELETE",
        headers: {
  ...
  const handleDeleteOne = async () => {
    try {
      const response = await fetch("/api/delete-Improve-product", {
        method: "DELETE",
  ...
  const filterAndSortReceiveByDate = (data: typeof checkStock) => {
    console.log(data);
  ...
  // ! Temporarily bypassing filterAndSortReceiveByDate, directly assigning checkStock
  // const filteredReceive = filterAndSortReceiveByDate(checkStock);
  const filteredReceive = checkStock
    ...
    headers: [
      <input
        ...
        onChange={(e) => {
          ...
          if (e.target.checked) {
            setSelectedItems(checkStock.map((receieve) => receieve.REFNO));
          } else {
            setSelectedItems([]);
          }
        }}
        checked={selectedItems.length === checkStock.length}
      />,
      "ลำดับ",
      "เลขที่",
      "วันที่",
